...
У события заявки появляется ссылка на сотрудника (`employee_id`), признак
«человек или система» (`actor_type`) и подробности (`details`): «было →
стало» по статусу и сумме, состав правки черновика, у кого заявка теперь.
Имя строкой (`actor`) остаётся: сотрудника переименуют или удалят, а
история должна читаться и через год.
