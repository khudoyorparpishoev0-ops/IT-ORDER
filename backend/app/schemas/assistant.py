...
class MemoryOut(BaseModel):
    ...
    Все списки считаются запросами к базе, поэтому подсказки живут и при
    выключенном помощнике: кончился баланс — автодополнение осталось.

    Имён и сумм здесь нет ни у кого: это знание о материалах, а не о
    людях. «UTP Cat6 Outdoor — часто на объекте Регар» показывать можно
    всем; «Начиб заказывал его за 3 355 сомони» — никому без права
    видеть чужие заявки.
    """

    #: Что просят чаще всего (по объекту, если он выбран). До 5.
    frequent: list[MemoryItem] = []
    #: Что заказывал сам сотрудник, по частоте. До 5.
    mine: list[MemoryItem] = []
    #: Что он заказывал в последний раз, по свежести. До 5.
    recent: list[MemoryItem] = []
    #: Что заказывали на выбранном объекте. До 5.
    project: list[MemoryItem] = []


class RepeatIn(BaseModel):
    """«Как в прошлый раз»: слова человека и объект, если он выбран."""

    text: str = Field(default="", max_length=500)
    project_id: int | None = None


class RepeatLine(BaseModel):
    title: str
    quantity: int
    unit: str | None = None


class RepeatOption(BaseModel):
    """Найденный прошлый вариант. Заявка по нему не создаётся сама."""
    ...
    project: str
    days_ago: int
    lines: list[RepeatLine] = []
    #: Почему этот вариант предложен: «ваша заявка на этом объекте».
    reasons: list[str] = []


class RepeatOut(BaseModel):
    """Варианты повтора. Первый — самый вероятный, остальные под кнопкой
    «Другой вариант»."""
