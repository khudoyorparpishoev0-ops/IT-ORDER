...
/**
 * Отмечает, что карточку открыли. Тела у запроса нет: кто открыл —
 * решает сессия на сервере, панель сообщает только сам факт.
 *
 * Ответ ничего не возвращает и список заявок не трогает: перерисовывать
 * экран из-за собственного просмотра незачем. Своё имя человек увидит в
 * блоке «кто видел» при следующем открытии.
 */
export function useMarkViewed() {
  return useMutation({
    mutationFn: (id: number) =>
      api<void>(`/api/requests/${id}/viewed`, { method: 'POST' }),
  });
}
